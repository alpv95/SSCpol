...
int jetmodel(char *fg_path, double *argv, int *argblocks, double *IC_StokesTotal,
             double *S_StokesTotal, double *f_pol_IC, double *f_pol) // argc is integer number of arguments passed, argv[0] is program name, argv[1..n] are arguments passed in string format
{
    ...
    FILE *FGfile;
    FGfile = fopen(fg_path, "r"); // Bessel Functions for Synchrotron
...
int main()
{
    ...
    int res = jetmodel(fg_file, argv, argblocks, IC_StokesTotal, S_StokesTotal, f_pol_IC, f_pol);
    return res;
}